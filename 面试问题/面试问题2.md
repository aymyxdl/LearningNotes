...
答： 没有区别，但是要注意最后面加 .name的情况。（没有太大的区别，具体的区别是带括号和不带括号的优先级不一样）
...
箭头有2个意思，一是当作箭头函数使用（表示函数）
另一个是作为 return 使用（前提是右边不跟大括号，如果跟大括号，就表示这是个函数表达式）
...
所有通过 new Function() 得到的对象都是  实例对象
也只有通过 new Function() 的才是 实例对象
...
是因为 Fn.prototype本身就用 constructor 属性，并不会通过 Fn.prototype.__proto__ 去查找
