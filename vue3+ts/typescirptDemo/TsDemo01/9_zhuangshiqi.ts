...
@logClass
class HttpClientDemo {   // 这里会报错,不影响
  constructor() {
  }
...
let hcd: any = new HttpClientDemo()
console.log('====', hcd.url);
hcd.run()
...
@logClass2('hello')
class HttpClientDemo2 {   // 这里会报错,不影响,快速修复 tsconfig.json就行
  constructor() {
  }
...
let hcd2: any = new HttpClientDemo()
...
function logClass3(param: any) {
  return class extends param {
    public apiUrl: any = '我是修改后的'
...
let hcd3:any = new HttpClientDemo3()
...
@logClass4
class HttpClientDemo4 {   // 这里会报错,不影响,快速修复 tsconfig.json就行

  // 属性装饰器的使用和 类装饰器一样,用 @, 不过是在属性前面使用
...
let hcd4: any = new HttpClientDemo4('url一开始的值')
...
var hcd5: any = new HttpClientDemo5()
